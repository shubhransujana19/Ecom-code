...
def userReg(request):
    if request.method=="POST":
        ...
        frm=MyRegForm()
    return render(request, 'ecom/userReg.html',{'frm':frm})
...
def userLogin(request):
    if request.method=="POST":
        ...
        frm=LoginForm()
    context={'frm':frm}    
    return render(request, 'ecom/userLog.html',context)
...
def store(request):
    ...
    if request.user.is_authenticated:
        ...
        context={'products': products, 'cartItems': cartItems, 'fname':request.user.first_name}
    else:
        items = []
...
def cart(request):
	...
	context = {'items': items, 'order': order, 'cartItems': cartItems, 'fname':request.user.first_name}
...
def contactus(request):
	return render(request,'ecom/contactus.html',{'fname':request.user.first_name})	

def about(request):
	return render(request,'ecom/about.html', {'fname':request.user.first_name})	
...
def userChngPass(request):
    if request.user.is_authenticated:
        ...
        return render(request, 'ecom/userChngPass.html', {'frm':frm, 'fname':request.user.first_name})
...
def userChngProfile(request):
    if request.user.is_authenticated:
        ...
        return render(request, 'ecom/userChngProfile.html', {'frm':frm, 'fname':request.user.first_name})
    else:
        return HttpResponseRedirect('/userlog')
